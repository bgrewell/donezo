...
export function TimeSensitiveSection({ rows }: { rows: DueRow[] }) {
  return (
    <section>
      ...
      {rows.length === 0 ? (
        <p className="py-1 font-sans text-[0.85rem] text-ae-muted">
        ...
        <ul>
          {rows.map((row) => (
            ...
            >
              <span
                className={cn(
                  "inline-flex w-[100px] shrink-0 items-center justify-center rounded-ae border px-1 py-0.5 font-mono text-[0.62rem] uppercase tracking-label",
                  row.overdue
                    ? "border-ae-warn-dim text-ae-warn"
                    : "border-ae-line text-ae-muted"
                )}
              >
              ...
              {row.project && (
                <span className="flex shrink-0 items-center gap-1.5">
                  ...
                  <span className="max-w-[180px] truncate font-mono text-[0.64rem] uppercase tracking-label text-ae-muted">
                    {row.project.name}
                  </span>
                </span>
              )}
